...
const getTextareaClasses = (siteId: number) => {
  const isGenerating = isGeneratingSite(siteId);
  const isFinished = isGenerationFinished(siteId);
  const isChanged = hasSiteChanges(siteId);
  const isEmpty = !altTexts[siteId]?.length;

  return {
    'border border-green-500 bg-green-50/40 text-ap-dark-green': isGenerating,
    'border textarea-border-flash': !isGenerating && isFinished,
    'border border-ap-light-green': !isGenerating && !isFinished && isChanged,
    'border border-ap-red': !isGenerating && !isFinished && !isChanged && isEmpty,
    'border border-transparent': !isGenerating && !isFinished && !isChanged && !isEmpty,
  };
};
...
<template>
  ...
  >
    <div class="relative h-32 w-full">
      ...
      <div v-if="cpTrigger" class="absolute bottom-2 left-2">
        <a
      ...
      <div
        class="absolute top-2 right-2 bottom-2 left-2 flex items-center justify-between rounded-2xl p-4 transition-all duration-300 ease-out"
        :class="[
          hasChanges
            ? 'pointer-events-auto bg-ap-light-green opacity-100'
            : 'pointer-events-none bg-ap-light-green opacity-0',
        ]"
      >
        <button
    ...
    <div class="flex w-full gap-0 pt-2">
      <div
        v-for="site in sites"
        :key="site.id"
      ...
      >
        <div class="relative flex w-full items-center px-3">
          ...
          <div
            class="absolute left-1/2 -translate-x-1/2 text-center text-[#AEAEAE]"
            :class="charactersRemaining(site.id) < 0 ? 'text-ap-red' : ''"
          >
            {{ charactersRemaining(site.id) }}
          </div>

          <button
            class="mb-1 ml-auto rounded-full border p-1 text-ap-dark-green transition-colors hover:bg-ap-light-green/30"
            :class="{
        ...
        <div class="w-full px-3 py-1">
          <textarea
            ...
            class="w-full resize-none rounded-lg border-transparent px-2 py-1 text-base leading-[1.1] text-[#555] transition-colors hover:border-ap-light-green focus:border focus:border-ap-light-green focus:ring-0 focus:outline-none"
            :class="getTextareaClasses(site.id)"
            rows="4"
          />
        </div>
      </div>
...
.textarea-border-flash {
  animation: textarea-border-flash 0.6s ease-in-out 2;
}

@keyframes textarea-border-flash {
  0%,
  100% {
