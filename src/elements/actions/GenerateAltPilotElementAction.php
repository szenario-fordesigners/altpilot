...
class GenerateAltPilotElementAction extends ElementAction
{
    ...
    {
        return Craft::t('altpilot', 'Generate Alt Text');
    }
    ...
    public function performAction(Craft\elements\db\ElementQueryInterface $query): bool
    {
        if (!Craft::$app->getUser()->checkPermission('accessPlugin-altpilot')) {
            $this->setMessage(Craft::t('altpilot', 'You do not have permission to use AltPilot.'));
            return false;
        }
        ...
        foreach ($elements as $element) {
            ...
            if ($asset === null) {
                Craft::warning('Unable to queue alt text generation: asset not found for ID ' . $element->id, 'altpilot');
                continue;
            }
        ...
        if ($jobCount > 0) {
            $message = Craft::t('altpilot', '{count, plural, =1{1 AltPilot job} other{# AltPilot jobs}} added to queue', ['count' => $jobCount]);
