...
class OpenAiService extends Component
{
    ...
    public function generateAltTextForImage(string $imageData, ?Asset $asset = null, ?Site $site = null): string
    {
        ...
        if (is_object($response) && method_exists($response, 'meta')) {
            try {
                ...
                Craft::warning('Could not access response meta information: ' . $e->getMessage(), 'altpilot');
            }
        }
    ...
    private function preparePrompt(string $promptTemplate, ?Asset $asset, ?Site $site): string
    {
        try {
            ...
            // Integrate optional role as instruction 7.
            if (!empty($settings->openAiPromptRole)) {
                $renderedRole = Craft::$app->getView()->renderObjectTemplate($settings->openAiPromptRole, $objectContext, [
                    'asset' => $asset,
                    'site' => $site,
                ]);

                $roleContext = trim($renderedRole);
                if (!empty($roleContext)) {
                    $finalPrompt .= "\n7. Interpret the image from this perspective: {$roleContext}.";
                    $finalPrompt .= "\n7a. Keep this perspective internal; do not mention the role, perspective, persona, or point of view in the final alt text.";
                }
            }

            // Integrate optional focus as instruction 8.
            if (!empty($settings->openAiPromptFocus)) {
                $renderedFocus = Craft::$app->getView()->renderObjectTemplate($settings->openAiPromptFocus, $objectContext, [
                    'asset' => $asset,
                    'site' => $site,
                ]);

                $focusContext = trim($renderedFocus);
                if (!empty($focusContext)) {
                    $finalPrompt .= "\n8. Pay particular attention to: {$focusContext}";
                }
            }

            return $finalPrompt;
        } catch (Throwable $e) {
            Craft::warning('Failed to render OpenAI prompt template: ' . $e->getMessage(), 'altpilot');
